function [XemburnSave, XemburnSave2, X3, X4] = SimulationRunOU(H,bb,aa)
...
X = initial;
OU1 = 10*randn(Nodes,1);
OU2 = 10*randn(Nodes,1);
...
% Xn = [ones(size(Xemburn,2), 1), t(50000:end-1)'];
% [b, bint, r, rint, stats] = regress(zscore(Xemburn)', Xn);
% 
% threshold = 0.0099;
% 
% % Check the sign of the regression coefficient
% if b(2) > threshold
% Xemburn = resample(Xemburn,fs_conv,fs_sim); % resamples has an antialiasing filtering 
% XemburnSave = Xemburn(:,(40*fs_sim):(N-(90*fs_sim)-1)); 
...
% XemburnSave2 = Xemburn2(:,(40*fs_sim):(N-(90*fs_sim)-1));   
% X3 = 100*rand(1,3713);
% X4 = 100*rand(1,3713);
% 
