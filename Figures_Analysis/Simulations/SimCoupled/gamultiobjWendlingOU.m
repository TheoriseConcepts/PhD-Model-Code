...
e0 = params1(11); ss = params1(12); K1 = params1(13); K2 = params1(14); tau = params1(15); D = params1(16);
params.A = A;
params.a=  a;
params.B = B;
params.b = b;
...
Gg = params.G*params.g;
    ...
    OU1(:,1) = OU1(:,1) - (OU1(:,1)/params.tau)*dt + (sqrt(2*params.D)/params.tau)*dW1(:,j); 
    OU2(:,1) = OU2(:,1) - (OU2(:,1)/params.tau)*dt + (sqrt(2*params.D)/params.tau)*dW2(:,j); 
    ...
    X(:,7) = X(:,7) + (Aa*(params.P+C2*2*params.e0./(1+exp(params.r*(params.v0-(C1*X(:,1))))) + K2*Xem2(:,j)) - 2*params.a*X(:,7) - X(:,2)*a2)*dt + Aa*params.ss*OU1(:,1);
    ...
    X(:,14) = X(:,14) + X(:,19)*dt;
    X(:,15) = X(:,15) + X(:,20)*dt;
    X(:,16) = X(:,16) + (Aa*(2*params.e0./(1+exp(params.r*(params.v0-(X(:,12)-X(:,13)-X(:,14)))))) - 2*params.a*X(:,16) - X(:,11)*a2)*dt;
    X(:,17) = X(:,17) + (Aa*(params.P+C2*2*params.e0./(1+exp(params.r*(params.v0-(C1*X(:,11))))) + K1*Xem1(:,j)) - 2*params.a*X(:,17) - X(:,12)*a2)*dt + Aa*params.ss*OU2(:,1);
    ev = 2*params.e0./(1+exp(params.r*(params.v0-(C3*X(:,11)))));
    X(:,18) = X(:,18) + (Bb*(C4*ev) - 2*params.b*X(:,18) - X(:,13)*b2)*dt;
    X(:,19) = X(:,19) + (Gg*C7*(2*params.e0./(1+exp(params.r*(params.v0-(C5*X(:,11)-X(:,15)))))) - 2*params.g*X(:,19) - X(:,14)*g2)*dt;
    X(:,20) = X(:,20) + (Bb*(C6*ev) - 2*params.b*X(:,20) - X(:,15)*b2)*dt;
