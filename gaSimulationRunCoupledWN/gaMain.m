...
if any(nn==1:400)
SimOptions = {'Control','White','Extended',Objectives}; % nn = 1:400
elseif any(nn==401:800)
SimOptions = {'PTZ','White','Extended',Objectives}; % nn = 401:800
end
...
%%
[ParamsTest,paramnames]= ParamsSetup(SimOptions{2},SimOptions{3},Nodes);

paramstoest=1:2*size(paramnames,2);
npars_est = length(paramstoest);

[paramsvec,paramDefCell,lb,ub] = setparsdefault(ParamsTest,paramstoest,npars_est);

if strcmp('White',SimOptions{2})
ff = @(x) ObjFitWhite(x, paramsvec, Data_z, paramstoest, T, bb, aa, SimOptions{4});
resetparsdefault;

elseif strcmp('OU',SimOptions{2})
    ff = @(x) ObjFitOU(x, paramsvec, Data_z, paramstoest, T, bb, aa, SimOptions{4});
resetparsdefault;
...
options2 = options1;
%options2.MaxTime = 60*60*15.75;% set max time to 24 hours - using a wallclock of 24hrs on isca
