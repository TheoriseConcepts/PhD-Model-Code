...
if uniform
    filter = dt*h*exp(-0.5*((t - mean(t)).^2)/(sigma2));
    i = filter < dt*h*1.e-6;
    filter(i) = [];
    zfilt = fconv(z,filter);        
    onesToFilt = ones(size(z));     % remove edge effect from conv 
    onesFilt = fconv(onesToFilt,filter); 
    ...
    for i = 1:n
        filter = h*exp(-0.5*((t - t(i)).^2)/(sigma2));
    ...
    for i = 1:nedge
        ...
        filter = h*exp(-0.5*((t(include) - t(iedge(i))).^2)/(sigma2));
